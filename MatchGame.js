...
class MatchGame extends Component {
  ...
  onClickToChangeImages = id => {
    this.setState({initial: id})
  }

  toClickTheImage = id => {
    const {imgUrl, score, show} = this.state

    if (id === imgUrl.id) {
      const random = Math.ceil(Math.random() * imagesList.length) - 1
      this.setState(prevState => ({
        score: prevState.score + 1,
        imgUrl: imagesList[random],
      }))
    } else {
  ...
  iWantToPlayAgain = () => {
    const {imgUrl, score, show, time} = this.state
    const newR = Math.ceil(Math.random() * imagesList.length) - 1
    this.setState({show: true, imgUrl: imagesList[newR], score: 0, time: 60})
    this.timerId = setInterval(this.tick, 1000)
  }
  ...
  render() {
    const {initial, imgUrl, score, show, time} = this.state
    const filteringImages = imagesList.filter(each => each.category === initial)
    const t = this.stopTheGame
    if (time === 0) {
      clearInterval(this.timerId)
    }
    return (
      <div>
        ...
        <div className="card2">
          {show ? (
            <div className="card3">
              <img className="img-height" src={imgUrl.imageUrl} alt="match" />
              ...
              <ul className="buttons">
                {tabsList.map(eachTab => (
                  <TabList
                    nameList={eachTab}
                    key={eachTab.tabId}
                    onClickToChangeImages={this.onClickToChangeImages}
                    isActive={initial === eachTab.tabId}
                  />
                ))}
              </ul>

              <ul className="images-list">
                {filteringImages.map(eachImg => (
                  <ImageList
                    details={eachImg}
                    key={eachImg.id}
                    toClickTheImage={this.toClickTheImage}
                  />
                ))}
              </ul>
            </div>
            ...
            <div className="game-over">
              <img
                ...
                alt="trophy"
              />
              <p className="y-score">YOUR SCORE</p>
              <p className="score">{score}</p>
              <button onClick={this.iWantToPlayAgain} className="play-btn">
                <img
                  className="reset"
